...
Final integration validation: 237 release-mode tests passed (two opt-in skips), GitHub CI passed on `b77265e`, and all 48 real-audio comparisons passed through the production service with Apple fallback disabled. The Developer ID signed app replayed a copied 31.4-second recording through its UI; transcript text and Copy matched the installed baseline exactly. That single UI smoke test included model preparation and is not a latency benchmark. The original installed app and real history were restored after the isolated test.
